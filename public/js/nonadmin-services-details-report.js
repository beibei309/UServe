(() => {
    const config = document.getElementById('servicesDetailsReportConfig');
    const trigger = document.querySelector('[data-report-user-trigger]');
    ...
    if (!config || !trigger || !modal || !form || !feedback || !submitBtn || !targetIdInput || !targetNameEl) {
        return;
    }
    ...
    const openModal = () => {
        targetIdInput.value = trigger.dataset.targetUserId || '';
        targetNameEl.textContent = trigger.dataset.targetUserName || '-';
