            ...
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-purple-100 text-sm">
                        {{ isset($category) ? 'Update category information and settings' : 'Add a new service category to the system' }}
                    </p>
                </div>
            </div>
        </div>
        ...
        <div class="px-6 py-4">
            <div class="flex items-center gap-4">
                <div class="w-12 h-12 rounded-xl flex items-center justify-center"
                     style="background-color: {{ $category->hc_color ?? '#3b82f6' }}20; color: {{ $category->hc_color ?? '#3b82f6' }};">
                    <i class="{{ $category->hc_icon ?? 'fas fa-folder' }} text-lg"></i>
                </div>
                <div>
                    <h2 class="font-bold text-lg transition-colors duration-300" style="color: var(--text-primary);">{{ $category->hc_name }}</h2>
                    <p class="text-sm transition-colors duration-300" style="color: var(--text-muted);">Slug: {{ $category->hc_slug }} • 
                        <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium
                            {{ $category->hc_is_active ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">
                            {{ $category->hc_is_active ? 'Active' : 'Inactive' }}
                        </span>
                    </p>
                </div>
            </div>
                ...
                <div class="p-6 sm:p-8">
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <p class="text-xs mt-2 transition-colors duration-300" style="color: var(--text-muted);">Edit the UServe service category name</p>
                        </div>
                        ...
                        <div>
                            ...
                            <textarea name="description" rows="4" 
                                      ...
                                      style="background-color: var(--bg-primary); border-color: var(--border-color); color: var(--text-primary);" 
                                      placeholder="Describe what this category is for...">{{ old('description', $category->hc_description ?? '') }}</textarea>
                        </div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    <div class="flex-1">
                                        <p class="text-xs font-medium transition-colors duration-300" style="color: var(--text-primary);" id="colorValue">{{ old('color', $category->hc_color ?? '#3b82f6') }}</p>
                    ...
                    <div class="mb-6">
                        ...
                        <div class="flex items-center gap-4 p-4 rounded-xl border transition-all duration-300" 
                            ...
                            <div class="w-16 h-16 flex items-center justify-center rounded-xl"
                                 style="background-color: {{ old('color', $category->hc_color ?? '#3b82f6') }}20; color: {{ old('color', $category->hc_color ?? '#3b82f6') }};">
                                <i id="previewIcon" class="{{ old('icon', $category->hc_icon ?? 'fa fa-folder') }} text-2xl"></i>
                            </div>
                            <div class="flex-1">
            ...
            <div class="p-6 sm:p-8">
                <div class="flex flex-col sm:flex-row gap-4 justify-between items-center">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h3 class="font-bold text-lg transition-colors duration-300" style="color: var(--text-primary);">Ready to save?</h3>
                            <p class="text-sm transition-colors duration-300" style="color: var(--text-muted);">Review your category information before saving</p>
                        </div>
                    </div>
