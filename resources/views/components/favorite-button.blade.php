@props(['serviceId' => null, 'userId' => null, 'isFavorited' => false])

@php
    $favoriteId = $serviceId ?? $userId;
@endphp

<button
    data-favorite-button
    id="favorite-btn-{{ $favoriteId }}"
    ...
    data-user-id="{{ $userId ?? '' }}"
>
    <svg id="favorite-icon-{{ $favoriteId }}" class="h-5 w-5 {{ $isFavorited ? 'fill-current text-red-600' : 'text-gray-700' }}" fill="{{ $isFavorited ? 'currentColor' : 'none' }}" viewBox="0 0 24 24" stroke="currentColor">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
    </svg>
    <span id="favorite-text-{{ $favoriteId }}" class="ml-2">
